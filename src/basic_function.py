...
def fitness_fun1(rf,pf,inp):
    try:
        ...
        # res = getulp(inp)/getulp(b)
        # res = 1.0/getulp(b)
        if (res == 0.0)|(np.isnan(res))|(res>np.finfo(np.double).max)|(np.isnan(b)):
            res = 1.0
        else:
            res = 1.0/res
        return float(res)
...
def fitness_fun(rf, pf, inp):
    try:
        ...
        r_val = float(rf(inp))
        p_val = float(pf(inp))
        # if (np.isnan(r_val))|(np.isnan(p_val)):
            # return 1.0
        if math.fabs(p_val) > np.finfo(np.double).max:# 大于1.7976931348623157e+308
            return 1.0
        # if math.fabs(inp) > np.finfo(np.double).max:# 大于1.7976931348623157e+308
            # return 1.0
...
def gen_all_bounds():
    ...
    for i in range(-1022,1023):
        bounds.append([pow(2.0,i),pow(2.0,i+1)])
        bounds.append([-pow(2.0,i+1),-pow(2.0,i)])
    return bounds
"""
def getFPEXP(x):
  # return x
  DBL_EXPOMASK = 0x7FF0000000000000;
  int_x = floatToRawLongBits(x)
  return ((int_x & DBL_EXPOMASK) >> 52)
x_l = gen_all_bounds()
x_t = x_l[2403]
list_x = list(np.random.uniform(x_t[0],x_t[1],8))
list_x.append(x_t[0])
list_x.append(x_t[1])
for i in list_x:
    print(getFPEXP(i))
"""
# 得到x的ulp(最接近x的两个浮点数的距离)
def getulp(x):
    x = float(x)
    k = math.frexp(x)[1]-1 #frexp(16)=(0.5,5) 是一个元组 即16=0.5*2^5
    if x == 0.0:
        return pow(2.0, -1074)
    if (k<1023)&(k>-1022):
        return pow(2.0,k-52)
...
#record_res_l：需要打印的列表, file_name：xls文件的目录, fnm：传入gsl函数名字
def output_err(t_l,name,name2):
    book = xlwt.Workbook()#创建一个book，可以为其设置编码
    sheet = book.add_sheet("sheet1")#创建一个sheet
    sheet.write(0, 0, "functions")#行，列，值
    ...
    n = 1
    for t in t_l:
        sheet.write(n,0,name2)#在第n+1行第一列写入函数名
...
def output_interval(t_l,name,name2):#修改对应创建的列表即可
    book = xlwt.Workbook()#创建一个book，可以为其设置编码
    sheet = book.add_sheet("sheet1")#创建一个sheet
    sheet.write(0, 0, "functions_name")#行，列，值
    sheet.write(0, 1, "max_error")
    sheet.write(0, 2, "input")
    sheet.write(0, 3, "interval")
    sheet.write(0, 4, "execute time")
    sheet.write(0, 5, "relative_err")
    n = 1
    for t in t_l:
        sheet.write(n,0,name2)#在第n+1行第一列写入函数名
        for k in range(0,len(t)):
...
def getUlpError(a,b):
    try:
        ia = floatToRawLongBits(np.abs(a))
        ib = floatToRawLongBits(np.abs(b))
        zo = floatToRawLongBits(0.0)
        if sign(a)!=sign(b):
            res = float(np.abs(ib-zo))+float(np.abs(ia-zo))
        else:
            res = float(np.abs(ib-ia))
        return int(res+1)
    except (ValueError, ZeroDivisionError, OverflowError, Warning,TypeError):
        return 1.0
...
print(getFPNum(1,2))
print("---------get 1/FPNum")
print(1/getFPNum(1,2))
def f_1(x):
    return 1
def f_2(x):
    return 2
print("-------fitness_fun")
print(fitness_fun(f_1,f_2,2))
"""